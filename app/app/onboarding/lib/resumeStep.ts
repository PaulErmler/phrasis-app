 ...
 * index. It is the persisted order again: no step is asked out of turn
 * since the politeness question was withdrawn.
 */
export const FLOW_ORDER: StepId[] = PROGRESS_STEP_ORDER;
...
export function resumeStepId(
  ...
  // test with the slider. `completeOnboarding` is idempotent, so users whose
  // course already exists (old flow got past customizing) just re-confirm
  // the mode and finish. Rows at 9+ never reach here. They graduate out
  // first.
