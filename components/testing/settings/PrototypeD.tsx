...
export function PrototypeD({ baseLanguages: baseProp, targetLanguages: targetProp }: PrototypeDProps) {
  ...
  const moveBaseUp = (idx: number) => setBaseLanguages((prev) => swap(prev, idx, idx - 1));
  const moveBaseDown = (idx: number) => setBaseLanguages((prev) => swap(prev, idx, idx + 1));
  const moveTargetUp = (idx: number) => setTargetLanguages((prev) => swap(prev, idx, idx - 1));
  const moveTargetDown = (idx: number) => setTargetLanguages((prev) => swap(prev, idx, idx + 1));
      ...
      <div className="space-y-2">
        ...
        <div className="flex flex-col items-center gap-0 py-2">
          ...
          {baseLanguages.map((code, idx) => {
            ...
            return (
              <div key={`base-${code}`} className="w-full flex flex-col items-center">
                <TimelineLanguageCard
                  code={code}
                  type="base"
                  plays={plays}
                  repPause={settings?.repPause ?? DEFAULT_PAUSE_BETWEEN_REPETITIONS}
          ...
          {baseLanguages.length > 0 && targetLanguages.length > 0 && (() => {
            const lastBasePlays =
              langSettings[baseLanguages[baseLanguages.length - 1]]?.plays ?? DEFAULT_REPETITIONS_BASE;
            const firstTargetPlays =
              langSettings[targetLanguages[0]]?.plays ?? DEFAULT_REPETITIONS_TARGET;
            return (
              <StepperPauseConnector
          ...
          {targetLanguages.map((code, idx) => {
            ...
            return (
              <div key={`target-${code}`} className="w-full flex flex-col items-center">
                <TimelineLanguageCard
                  code={code}
                  type="target"
                  plays={plays}
        ...
        {plays > 1 && (
          <div className="space-y-1">
            <div className="flex items-center justify-between">
              <span className="text-xs text-muted-foreground">Pause between plays</span>
              <span className="text-xs font-medium tabular-nums">{repPause}s</span>
            </div>
            <Slider
              min={0}
              max={30}
