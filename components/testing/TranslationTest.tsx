...
export function TranslationTest() {
  ...
  const [requestId, setRequestId] = useState<Id<'translationRequests'> | null>(
    null,
  );
  const [displayedResult, setDisplayedResult] = useState('');

  const requestTranslation = useMutation(
    api.testing.translation.requestTranslation,
  );
  ...
  useEffect(() => {
    if (
      ...
      setDisplayedResult(
        `Error: ${translationRequest.error || 'Translation failed'}`,
      );
    }
  }, [translationRequest]);
  ...
  const isTranslating =
    requestId !== null && translationRequest?.status === 'pending';
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Test the Google Cloud Translation API</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {displayedResult && (
          <div
            className={`rounded-lg p-3 ${displayedResult.startsWith('Error:') ? 'bg-destructive/10' : 'bg-muted'}`}
          >
            <p className="text-muted-xs mb-1">
              {displayedResult.startsWith('Error:') ? 'Error:' : 'Translation:'}
            </p>
            <p
              className={`text-sm ${displayedResult.startsWith('Error:') ? 'text-destructive' : ''}`}
            >
              {displayedResult.startsWith('Error:')
                ? displayedResult.slice(7)
                : displayedResult}
            </p>
          </div>
        )}
