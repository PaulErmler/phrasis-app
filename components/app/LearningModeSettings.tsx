...
export function LearningModeSettings({
  ...
  const baseLanguages = resolveLanguageOrder(courseSettings.baseLanguageOrder, baseProp);
  const targetLanguages = resolveLanguageOrder(courseSettings.targetLanguageOrder, targetProp);
  ...
  const handleRepetitionPauseChange = async (language: string, value: number) => {
  ...
  const moveBaseUp = (idx: number) => {
    ...
    void updateSettings({ courseId: courseSettings.courseId, baseLanguageOrder: next });
  };
  ...
  const moveTargetUp = (idx: number) => {
    ...
    void updateSettings({ courseId: courseSettings.courseId, targetLanguageOrder: next });
  };
      ...
      <SheetContent side="right" className="w-full sm:max-w-[380px] p-0 [&>button:last-child]:hidden">
        ...
        <div className="px-6 py-4 space-y-4 overflow-y-auto max-h-[calc(100vh-3.5rem)]">
          ...
          <div className="flex flex-col items-center gap-0 py-1">
              ...
              return (
                <div key={`base-${code}`} className="w-full flex flex-col items-center">
                  <TimelineLanguageCard
                    ...
                    onRepPauseChange={(v) => handleRepetitionPauseChange(code, v)}
            ...
            {autoAdvance && (baseLanguages.length > 0 || targetLanguages.length > 0) && (
              <StepperPauseConnector
                label={t("pauseBeforeAutoAdvance")}
                seconds={courseSettings.pauseBeforeAutoAdvance ?? DEFAULT_PAUSE_BEFORE_AUTO_ADVANCE}
