...
export function NoCollectionState({ onGoHome }: NoCollectionStateProps) {
  ...
  return (
    <main className="flex-1 flex flex-col items-center justify-center gap-4 px-4">
      <p className="text-muted-foreground text-center">{t("empty.noCollection")}</p>
  ...
  return (
    <main className="flex-1 flex flex-col items-center justify-center gap-6 px-4">
      <div className="text-center space-y-2">
        <p className="text-lg font-medium">{t("empty.noCardsDue")}</p>
        <p className="text-sm text-muted-foreground">{t("empty.allDone")}</p>
      </div>
      <Button
        size="lg"
        onClick={onAddCards}
