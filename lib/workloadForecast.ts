...
export function deriveReturnKernels(opts: {
  ...
  const reviewKernel = (mix: MatureStabilityMix): number[] => {
...
export function buildWorkloadForecast(
  ...
  const days: WorkloadDay[] = scheduledDays.map((s, d) => {
    ...
    // sightings.
    const whatIfEvents =
      d === 0 ? whatIfAdds * newCardEvents : whatIfAdds * (1 + rates.pAgain);
    return {
      offset: d,
      scheduled: {
        backlog: s.backlog,
  ...
  return {
    ...
    rates: {
      ...
      avgDailyMinutes:
        history.activeDays > 0 ? gradedTimeMs / history.activeDays / 60_000 : 0,
      secondsPerReview,
      sampleSize:
