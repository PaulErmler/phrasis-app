...
export function createHomeTour(
  ...
  const freePlayStep =
    ctx?.reviewMode === 'full'
      ? tourStep(t, 'home.freeStudyMode', tutorialSelector(TUTORIAL_ANCHORS.freeStudyMode), 'bottom', 'center')
      : tourStep(t, 'home.radioMode', tutorialSelector(TUTORIAL_ANCHORS.radioMode), 'bottom', 'center');
  ...
  const steps: AppDriveStep[] = [
    ...
    tourStep(t, 'home.learnNew', tutorialSelector(TUTORIAL_ANCHORS.learnNew), 'bottom', 'center'),
    tourStep(t, 'home.learnAndReview', tutorialSelector(TUTORIAL_ANCHORS.learnAndReview), 'bottom', 'center'),
    freePlayStep,
    tourStep(t, 'home.reviewModeToggle', tutorialSelector(TUTORIAL_ANCHORS.reviewModeToggle), 'bottom', 'center'),
    tourStep(t, 'home.contentSource', tutorialSelector(TUTORIAL_ANCHORS.contentSourceFilter), 'bottom', 'center'),
    ...
    ...(ctx?.hideDueCounts === true
      ? []
      : [tourStep(t, 'home.dueCounts', tutorialSelector(TUTORIAL_ANCHORS.dueCounts), 'bottom', 'center')]),
    // The rotating forecast in the progress card ("by the end of the year…").
    tourStep(t, 'home.projections', tutorialSelector(TUTORIAL_ANCHORS.projections), 'bottom', 'center'),
    ...
    ...(ctx?.hideWorkloadForecast === true
      ...
      : [
          {
            ...tourStep(t, 'home.workload', tutorialSelector(TUTORIAL_ANCHORS.workloadForecast), 'top', 'center'),
            skipIfMissing: true,
          } satisfies AppDriveStep,
        ]),
    tourStep(t, 'home.difficultySelection', tutorialSelector(TUTORIAL_ANCHORS.collectionCarousel), 'top', 'center'),
    ...
    tourStep(t, 'home.readyToLearn', tutorialSelector(TUTORIAL_ANCHORS.learnAndReview), 'bottom', 'center'),
  ];
