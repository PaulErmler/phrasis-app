...
**Free trials are switched off** (2026-09) via the `FREE_TRIAL` constant in `autumn.config.ts`, which is spread into `basic`/`pro`/`ultra` only when non-null. Customers who were mid-trial at the push keep it: they sit on the old plan version, and their Stripe subscription already carries its `trial_end`. Nothing was deleted, so turning trials back on is: restore `FREE_TRIAL`, flip `TRIALS_ENABLED` in `components/landing/pricing-section.tsx` (the only hardcoded trial copy), un-skip the trial journeys in `e2e/billing.spec.ts` and `e2e/billing-clock.spec.ts` (A, B, D), and `atmn push`. The in-app pricing table needs no change either way: its trial badge and "Start Free Trial" CTA both key off Autumn's `has_trial`. The same change cut Free's starter credits to 100 and raised its monthly audio regenerations to 40. `free` is meant to be MIGRATED rather than versioned on that push, so existing free users take both; the paid plans stay on create-version so nobody mid-trial is moved.
