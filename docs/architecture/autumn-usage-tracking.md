...
Since the credits rollout, `custom_sentences`, `translation_auto_fill`, and `chat_messages` are no longer granted as separate plan items. Instead, plans grant a shared `credits` pool (Free: 100 one-off + 30/month; Basic: 430/month; Pro: 1,030/month; Ultra: 3,030/month) and the `credits` feature in `autumn.config.ts` declares a `creditSchema` mapping each of those features to a credit cost (currently 1 credit per unit each). Monthly credits reset with the billing cycle (no rollover); one-off grants persist.
...
**Starter credits stay on the free plan.** Since `free` is `autoEnable`, every customer is attached to it at creation and receives the 100 one-off grant once. A customer who subscribes to a paid tier no longer holds `free` (verified in sandbox: a `pro_annual` customer's `subscriptions` contains only that plan), so paid-from-day-one subscribers get no starter credits, which is accepted. Do NOT "fix" this by adding a `one_off` credits item to Basic/Pro/Ultra: switching plans ends one entitlement and creates a new one, so that would grant a fresh 100 on every upgrade.
...
**Free trials are switched off** (2026-09). The switch is `TRIALS_ENABLED` in `lib/constants/trials.ts`, and everything derives from it: the `FREE_TRIAL` object spread into `basic`/`pro`/`ultra` in `autumn.config.ts`, the hardcoded trial copy on the landing page (`components/landing/pricing-section.tsx`, the only hardcoded copy there is), and which half of each paired billing e2e suite runs. Turning trials back on is flipping that one constant and running `atmn push`. It lives in its own file because `autumn.config.ts` imports `atmn` and the pricing section is a client component, so the two cannot share a constant directly.

The in-app pricing table and the paywall need no change either way: their trial badge and "Start Free Trial" CTA both key off Autumn's `has_trial`. One place gets this wrong by keying off *eligibility* instead, and it is worth knowing about: `trialState.trialEligible` is true for every new customer whether or not a trial exists, so anything gated on it alone will behave as if trials are still on. Gate on `has_trial && trialEligible`, the pair `getCheckoutContent` uses.

Customers who were mid-trial at the push keep it: they sit on the old plan version, and their Stripe subscription already carries its `trial_end`.

**Conversion tracking moves tier on its own, and campaigns do not.** `OpenAIPixelConversions.tsx` picks `trial_started` vs `subscription_created` from `paid.isTrialing`, so with trials off new customers correctly fire `subscription_created` and no code change is needed. The consequence is operational: the `trial_started` conversion flatlines and `subscription_created` absorbs its volume. Any ad campaign optimising toward `trial_started` has to be repointed by hand.

**The e2e suites come in pairs**, gated by `trialSuite`/`noTrialSuite` in `e2e/trial-mode.ts` so exactly one of each pair runs. `billing.spec.ts` pairs with `billing-no-trial.spec.ts`; `billing-clock.spec.ts` journeys A/B/D pair with `billing-clock-no-trial.spec.ts` journeys A'/B'. Journey C runs in both modes (it reconstructs a legacy non-trial subscription), and so does `payment-overdue.spec.ts`, whose setup only needs a Stripe customer and a saved card. There is no D': journey D lapses a *trialing* subscription, which has charged nothing, and neither route open to a paid Managed Payments subscription reproduces that, so its surviving assertion is the last stage of B'.

**The allowance changes that shipped alongside** cut Free's starter credits to 100 and raised its monthly audio regenerations to 40. These two do NOT behave alike under a push and must not be reasoned about as one decision. The regeneration raise is a `month` item, so it recomputes on the next cycle. The credit cut is `one_off`, granted once at customer creation, and the standing rule above is `create_version` with no migration precisely so an already-granted balance is never re-baselined. **Before pushing this to prod, probe it**: `npx atmn push --headless` without `-y` analyses the diff and prints the exact flags without applying, then confirm on a sandbox free customer holding a *partially spent* starter balance what a `free` migration does to it. Write the answer here. The paid plans stay on `create_version` either way so nobody mid-trial is moved.
