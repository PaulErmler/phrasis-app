...
describe('EditCardDialog: accepted alternatives', () => {
  ...
  it('lists the alternatives under their language, below the card sentence, without touching base rows', () => {
    ...
    expect(screen.getByDisplayValue('Me gustaría un café.')).toBeInTheDocument();
    ...
    expect(screen.queryByTestId('edit-alternatives-en')).not.toBeInTheDocument();
    // Exactly one delete icon per alternative — none for the sentences.
    expect(screen.getAllByRole('button', { name: 'deleteAlternative' })).toHaveLength(2);
  });
  ...
  it('marks a row deleted (undoable) and deletes it on save', async () => {
    const onOpenChange = renderDialog();
    const [firstDelete] = screen.getAllByRole('button', { name: 'deleteAlternative' });
    fireEvent.click(firstDelete);
    // Undo affordance appears; the input is disabled until save or undo.
    expect(screen.getByRole('button', { name: 'undoDeleteAlternative' })).toBeInTheDocument();
  ...
  it('undo restores the row so save touches nothing', () => {
    ...
    const [firstDelete] = screen.getAllByRole('button', { name: 'deleteAlternative' });
    fireEvent.click(firstDelete);
    fireEvent.click(screen.getByRole('button', { name: 'undoDeleteAlternative' }));
