...
describe("backfillContentVersions migration", () => {
  it("stamps legacy audio rows at the baseline version (not the current per-language version)", async () => {
    ...
    const bumpedAudio = await insertAudio(t, textId, BUMPED_LANG);
    ...
    expect(getCurrentTtsVersion(BUMPED_LANG)).toBeGreaterThan(
      DEFAULT_CONTENT_VERSION,
    );
  });
  ...
  it("leaves the backfilled audio row stale so the ttsVersion bump regenerates it", async () => {
    const t = convexTest(schema, modules);
    const textId = await seedText(t);
    const audioId = await insertAudio(t, textId, BUMPED_LANG);
    ...
    expect(isTtsVersionStale(BUMPED_LANG, row?.ttsVersion)).toBe(true);
  });

  it("stamps legacy translation rows at the baseline version", async () => {
    const t = convexTest(schema, modules);
    const textId = await seedText(t);
    ...
    expect(row?.translationVersion).toBe(DEFAULT_CONTENT_VERSION);
    // No language bumps translationVersion today, so baseline === current and a
    // backfilled translation is NOT stale (no regeneration storm).
    expect(isTranslationVersionStale("es", row?.translationVersion)).toBe(false);
  });

  it("is idempotent: rows already carrying a version are left untouched", async () => {
    const t = convexTest(schema, modules);
    const textId = await seedText(t);
    const stampedAudio = await insertAudio(t, textId, BUMPED_LANG, 5);
