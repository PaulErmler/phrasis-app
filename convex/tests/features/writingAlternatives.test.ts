...
import {} from '../../db/translationReads';
...
describe('features/writingAlternatives generation pipeline', () => {
  ...
  it('getAlternativeContext voices the sentence in the voice it was corrected to', async () => {
    const t = convexTest(schema, modules);
    const a = await seedAlternative(t, 'A');
    await t.run(async (ctx) => {
      await ctx.db.patch(a.textId, { audioSpeakerGender: 'female' });
    });
    const context = await t.query(
      internal.features.writingAlternatives.getAlternativeContext,
      { alternativeId: a.alternativeId },
    );
    // The card hears a female voice (the sentence's, after the Flag
    // dialog's correction), so the alternative is probed female first.
