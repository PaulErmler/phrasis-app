...
// Aggregate component is not registered with convex-test here; stub it so the
// scheduling module loads without trying to wire `cardsByState` etc.
...
describe("content-source filter — getCardForReviewEmptyReason", () => {
  ...
  it("returns all_caught_up when filter is 'both' and nothing is due", async () => {
    ...
    await t.run(async (ctx) => {
      const future = Date.now() + 60_000;
      for (const cid of [f.premadeCard, f.editedPremadeCard, f.customCard, f.chatCard]) {
...
describe("content-source filter — backfillCollectionOrigin", () => {
  it("Phase A classifies premade / custom / chat correctly and is idempotent", async () => {
    const t = convexTest(schema, modules);
    ...
    const ids = await t.run(async (ctx) => {
      ...
      const premade = await ctx.db.insert("collections", {
        name: "L01",
        textCount: 0,
        datasetId,
      });
      const legacy = await ctx.db.insert("collections", {
        name: "A1",
        textCount: 0,
        legacy: true,
      });
      ...
      return { premade, legacy, chat, custom };
    });

    const res1 = await t.mutation(
      internal.admin.backfillCollectionOrigin.runCollectionsOriginBackfill,
      {},
    );
    expect(res1.classified.premade).toBe(2); // dataset + legacy
    expect(res1.classified.chat).toBe(1);
    expect(res1.classified.custom).toBe(1);

    const after = await t.run(async (ctx) => ({
      premade: await ctx.db.get(ids.premade),
      legacy: await ctx.db.get(ids.legacy),
      chat: await ctx.db.get(ids.chat),
      custom: await ctx.db.get(ids.custom),
    }));
    expect(after.premade?.origin).toBe("premade");
    expect(after.legacy?.origin).toBe("premade");
    expect(after.chat?.origin).toBe("chat");
    expect(after.custom?.origin).toBe("custom");

    // Idempotent — second run touches nothing.
    const res2 = await t.mutation(
      internal.admin.backfillCollectionOrigin.runCollectionsOriginBackfill,
      {},
    );
