...
function stubAutumn(routes: Record<string, unknown>) {
  const fetchMock = vi.fn(async (url: string, init: any = {}) => {
    ...
    const key = Object.keys(routes).find((k) => {
      ...
      return (
        (wantMethod === undefined || wantMethod === method) && url.includes(path)
      );
    });
...
describe("switchPlanDuringTrial", () => {
  ...
  it("reads the trial end from current_period_end (v1.2 leaves trial_ends_at null)", async () => {
    stubAutumn({
      ...
      "/checkout": { product: { scenario: "downgrade", properties: { is_free: false } } },
    });
    const t = convexTest(schema, modules);
  ...
  it("refuses to re-attach the plan already being trialed", async () => {
    stubAutumn({
      "/customers/": { products: [trialingProduct()] },
      "/checkout": { product: { scenario: "upgrade", properties: { is_free: false } } },
    });
    const t = convexTest(schema, modules);
    await expect(
  ...
  it("never shortens a trial below the API minimum of one day", async () => {
    stubAutumn({
      "/customers/": {
        products: [trialingProduct({ current_period_end: Date.now() + 3_600_000 })],
      },
      "/checkout": { product: { scenario: "upgrade", properties: { is_free: false } } },
      "/billing.attach": { payment_url: null },
    });
    const t = convexTest(schema, modules);
    await asUser(t).action(api.billing.switchPlanDuringTrial, { productId: "pro" });
    expect(
  ...
  it("a free target takes the scheduled path, never the immediate one", async () => {
    stubAutumn({
      "/customers/": { products: [trialingProduct()] },
      "/checkout": { product: { scenario: "cancel", properties: { is_free: true } } },
    });
    const t = convexTest(schema, modules);
    const res = await asUser(t).action(api.billing.switchPlanDuringTrial, {
      productId: "free",
  ...
  it("rejects an unclassifiable scenario rather than guessing", async () => {
    stubAutumn({
      "/customers/": { products: [trialingProduct()] },
      "/checkout": { product: { scenario: "active", properties: { is_free: false } } },
    });
    const t = convexTest(schema, modules);
    await expect(
      asUser(t).action(api.billing.switchPlanDuringTrial, { productId: "pro" }),
  ...
  it("renew of the trialed plan un-schedules a pending switch via plain legacy attach", async () => {
    stubAutumn({
      "/customers/": { products: [trialingProduct()] },
      "/checkout": { product: { scenario: "renew", properties: { is_free: false } } },
    });
    const t = convexTest(schema, modules);
    ...
    expect(attach.body).toEqual({ customer_id: USER, product_id: "basic_annual" });
    expect(callsTo("/billing.attach")).toHaveLength(0);
  });
  ...
  it("scenario 'new' takes the immediate billing.attach path like an upgrade", async () => {
    stubAutumn({
      "/customers/": { products: [trialingProduct()] },
      "/checkout": { product: { scenario: "new", properties: { is_free: false } } },
      "/billing.attach": { payment_url: null },
    });
    const t = convexTest(schema, modules);
    const res = await asUser(t).action(api.billing.switchPlanDuringTrial, {
  ...
  it("forwards a payment_url when Autumn demands a card despite the trial", async () => {
    stubAutumn({
      "/customers/": { products: [trialingProduct()] },
      "/checkout": { product: { scenario: "upgrade", properties: { is_free: false } } },
      "/billing.attach": { payment_url: "https://checkout.stripe.com/c/pay_123" },
    });
    const t = convexTest(schema, modules);
    const res = await asUser(t).action(api.billing.switchPlanDuringTrial, {
      productId: "pro",
...
describe("cancelOverdueSubscription", () => {
  ...
  it("cancels the past-due plan immediately (v1 status encoding)", async () => {
    stubAutumn({
      "GET /customers/": {
        products: [freeProduct, { ...trialingProduct(), id: "basic", status: "past_due" }],
  ...
  it("also detects the v2 boolean encoding of past_due", async () => {
    stubAutumn({
      "GET /customers/": {
        products: [
          freeProduct,
          { ...trialingProduct(), id: "basic", status: "active", past_due: true },
        ],
        invoices: [openInvoice],
      },
      "/cancel": { success: true },
    ...
    stubAutumn({
      "GET /customers/": {
        products: [
          ...
          { id: "extra", status: "past_due", is_default: false, is_add_on: true },
        ],
        invoices: [openInvoice],
      },
      "POST /customers": freeOnlyCustomer,
  ...
  it("plan reads past_due but every invoice is settled → recovered, never cancels", async () => {
    stubAutumn({
      "GET /customers/": {
        ...
        invoices: [{ status: "paid", hosted_invoice_url: "https://invoice.stripe.com/i/abc" }],
      },
      "POST /customers": freeOnlyCustomer,
    });
    const t = convexTest(schema, modules);
