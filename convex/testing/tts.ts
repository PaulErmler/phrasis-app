...
export const requestTTS = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.speed < MIN_TTS_SPEED || args.speed > MAX_TTS_SPEED) {
      throw new ConvexError(
        `Invalid speed. Must be between ${MIN_TTS_SPEED} and ${MAX_TTS_SPEED}`,
      );
    }
    ...
    await ctx.scheduler.runAfter(0, internal.testing.tts.processTTS, {
      requestId,
    });
...
export const processTTS = internalAction({
  ...
  handler: async (ctx, args) => {
    const request = await ctx.runQuery(
      internal.testing.tts.getRequestInternal,
      {
