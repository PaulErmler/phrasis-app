...
export const requestTranslation = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.scheduler.runAfter(0, internal.features.translation.processTranslation, {
      requestId,
    });
...
export const processTranslation = internalAction({
  ...
  returns: v.null(),
  handler: async (ctx, args) => {
    const request = await ctx.runQuery(internal.features.translation.getRequestInternal, {
    ...
    try {
      ...
      await ctx.runMutation(internal.features.translation.updateRequestResult, {
        requestId: args.requestId,
        status: "completed",
        result: translation,
      });
