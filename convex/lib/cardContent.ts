...
import { type RenderingText } from '../../lib/preferenceResolution';
...
export async function buildTextContentBatchForLanguages(
  ...
  const translationFetches: Array<{
    ...
    /**
     * The rendering the slot reads: the text's voice and the key its row
     * carries. Null for a reader that passed no rendering fields.
     */
    rendering: TextRendering | null;
    isSource: boolean;
  }> = [];
  const audioFetches: Array<{
    ...
    rowLang: string;
    textId: Id<'texts'>;
    /** The rendering key the slot's clip carries. */
  ...
  for (const input of inputs) {
    ...
    const rendering = hasRendering
      ? renderingOfText(input.renderingText!, input.textId)
      : null;
    for (const lang of allLanguages) {
      ...
      // included: its clip (and the accent row's) carries the same key.
      const sourceRendering = rendering;
      const sourceAudioKey = sourceRendering?.key;
  ...
  const chosen = rows.map((row) => ({ row }));
  // The "Retranslating" pill keys off the LLM claim of the row the card
  // reads. A non-stale claim means a job for that row is in flight; it does
  // NOT fire on "regenerate audio" (no LLM phase, no claim).
  ...
  // Target slots play the clip of the row they show.
  translationFetches.forEach((item) => {
    if (item.isSource) return;
    audioFetches.push({
      slot: `${item.key}:${item.lang}`,
      rowLang: item.lang,
      textId: item.textId,
      variantKey: item.rendering?.key,
    });
  });
  ...
  for (const input of inputs) {
    ...
    const voiceGender =
      input.renderingText !== undefined
        ? renderingOfText(input.renderingText, input.textId).voiceGender
        : undefined;
    const translations = allLanguages.map((lang) => {
      ...
      return {
        ...
        ...(opts?.markVersionStale
          ...
          : {}),
        // The chip: the sentence's voice, on every surface that shows a card.
        ...(voiceGender ? { voiceGender } : {}),
      };
    });
 ...
 *
 * `view` is the card's (`viewOfCard(card)`). Its pin
