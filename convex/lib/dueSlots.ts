...
/**
 * The learner's study-day boundary for a review, or undefined when day
 * snapping is off for them or the zone is unusable (then the review keeps
 * FSRS's exact instant rather than failing). Shared by `reviewCard` and
 * the snap sweep so the two can't disagree on the rule.
 */
 ...
 * active card of the deck holds on the given track, or null when sixteen
 * consecutive probes were all taken (callers keep the exact instant then).
 *
 * Random within the window (the order cards come up on the day is random,
 * by design), unique by construction: the candidate is checked with a point
 * lookup on the track's due index, and a hit moves the probe forward one
 * millisecond. The index is deck-scoped and the merge in
 * `fetchTrackDueCards` sorts by due date, so uniqueness here is what makes
 ...
 * stays unique too.
 *
 * Only active (not hidden, not mastered) cards are checked. That matches
