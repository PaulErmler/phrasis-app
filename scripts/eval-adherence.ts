 ...
 *   rewrite     the production mechanism for variants: the BASELINE output
 *               (the canonical wording) rewritten for the request with
 *               `buildRenderingRewritePrompt`, product wording. Judged on
 *               the same scale; its identity rate against the baseline is
 *               printed too, which is what the generate-and-compare rule
 *               pays for.
 *
...
const ARMS = ['baseline', 'product', 'literature', 'rewrite'] as const;
...
const CASES: Case[] = [
  ...
  {
    id: 'a6',
    text: 'Please wait here a moment.',
    addresses: true,
    first: false,
  },
  ...
  {
    id: 'd3',
    text: 'This restaurant is very good.',
    addresses: false,
    first: false,
  },
  {
...
const CHECKS: Record<string, Record<string, (text: string) => boolean>> = {
  ja: {
    plain: (t) =>
      !/(です|ます|ません|ました|でした|ましょう|ください)[。！？!?]*$/.test(
        t.trim(),
      ),
    'desu-masu': (t) =>
      /(です|ます|ません|ました|でした|ましょう|ください)[。！？!?]*$/.test(
        t.trim(),
      ) && !/(いたし|ござい|おり|いただ|くださ|なさ)/.test(t),
    keigo: (t) =>
      /(いたし|ござい|おり|いただ|くださ|なさ|ご.+?ください|お.+?ください)/.test(
        t,
      ),
  },
  ko: {
    ...
    hapsyo: (t) =>
      /(습니다|ㅂ니다|습니까|십시오|세요)[.!?]*$/.test(t.trim()) &&
      !/요[.!?]*$/.test(t.trim()),
  },
  de: {
    t: (t) =>
      /\b(du|dich|dir|dein|deine|deinen|deiner|kommst|willst|kannst|wohnst)\b/i.test(
        t,
      ) && !/\b(Sie|Ihnen|Ihr|Ihre|Ihren)\b/.test(t),
  ...
  ru: {
    t: (t) =>
      /\b(ты|тебя|тебе|тобой|твой|твоя|твои)\b/i.test(t) &&
      !/\b(вы|вас|вам|вами|ваш|ваша|ваши)\b/i.test(t),
    v: (t) =>
      /\b(вы|вас|вам|вами|ваш|ваша|ваши)\b/i.test(t) ||
      /(ите|ете|ёте)\b/.test(t),
  },
  es: {
...
function promptArgs(
  ...
  return {
    ...base,
    requestedForm: req.form
      ? { id: req.form.id, label: req.form.label, prompt: req.form.prompt }
      : undefined,
    requestedGender: req.gender,
    promptWording: arm === 'literature' ? 'literature' : 'product',
  };
}
...
async function main() {
  ...
  const langs = (argValue(argv, 'langs') ?? DEFAULT_LANGS.join(','))
    .split(',')
    .filter(Boolean);
  const arms = (argValue(argv, 'arms') ?? ARMS.join(','))
    .split(',')
  ...
  await pool(jobs, 4, async (job) => {
    ...
    for (const arm of arms) {
      ...
      if (arm === 'rewrite') {
        // The canonical wording is the baseline output (cached under its own
        // key, so a rewrite-only run still reuses it).
        const canonical =
          outputs.get('baseline') ??
          (await translate(
            openrouter,
            `tr|baseline|${job.lang}|${job.c.id}|${job.reqKey}`,
            promptArgs(job.lang, job.c, 'baseline', job.req),
          ));
        if (canonical === null) {
          outputs.set(arm, null);
          continue;
        }
        const args = promptArgs(job.lang, job.c, 'product', job.req);
        outputs.set(
          arm,
          await translate(
            openrouter,
      ...
      outputs.set(
        ...
        await translate(
          openrouter,
          key,
          promptArgs(job.lang, job.c, arm, job.req),
        ),
      );
    }
    const candidates = [
    ...
    for (const arm of arms) {
      ...
      results.push({
        ...
        text,
        mech,
        score: text !== null ? (scores[text] ?? null) : null,
      });
    }
  });
  ...
  out(
    `${'lang'.padEnd(6)} ${'request'.padEnd(18)} ${'arm'.padEnd(11)} ${'mech pass'.padEnd(11)} ${'judge mean'.padEnd(11)} n`,
  );
  for (const lang of langs) {
    const reqKeys = [
    ...
    for (const reqKey of reqKeys) {
      for (const arm of arms) {
        ...
        const mechPass = mech.length
          ? `${((100 * mech.filter((r) => r.mech).length) / mech.length).toFixed(0)}%`
          : '-';
        ...
        const mean = scored.length
          ...
          : '-';
        out(
          `${lang.padEnd(6)} ${reqKey.padEnd(18)} ${arm.padEnd(11)} ${mechPass.padEnd(11)} ${mean.padEnd(11)} ${list.length}`,
        );
      }
    }
  }
  if (arms.includes('rewrite') && arms.includes('baseline')) {
    out('\n== rewrite identity rate (rewrite output == baseline wording) ==');
    for (const lang of langs) {
      const pairs = results.filter(
        (r) => r.lang === lang && r.arm === 'rewrite' && r.text !== null,
      );
      const same = pairs.filter((r) =>
        results.some(
          (b) =>
            b.lang === lang &&
            b.arm === 'baseline' &&
      ...
      out(`  ${lang}: ${same}/${pairs.length}`);
    }
  }
  out('\n== worst cases (judge <= 4, non-baseline) ==');
  for (const r of results) {
    if (r.arm !== 'baseline' && r.score !== null && r.score <= 4)
