...
test.describe('course management', () => {
  test('archive a course', async ({ page }) => {
    ...
    if (!(await settingsIcon.isVisible().catch(() => false))) {
      ...
      await expect(page.getByTestId('course-settings').first()).toBeVisible({
        timeout: 10_000,
      });
    }
  ...
  test('create a new course', async ({ page }) => {
    ...
    await expect(async () => {
      ...
      const courseRow = page
        .locator(
          `[data-testid="course-menu-entry"][data-target-language="${targetCode}"]`,
        )
        .first();
      ...
      const fallbackByName = page
        ...
        .first();
      const fallbackVisible = await fallbackByName
        .isVisible()
        .catch(() => false);
