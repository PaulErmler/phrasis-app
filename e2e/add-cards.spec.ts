...
test.describe('add cards', () => {
  test('add-cards form renders and rejects empty submit', async ({ page }) => {
    ...
    const editor = page
      .getByRole('textbox', {
        name: /english|spanish|french|german|italian|portuguese|russian|hindi|chinese|japanese|korean|target|source/i,
      })
      .first();
    ...
    const submit = page
      .getByRole('button', {
        name: /^(save|add|create|generate|hinzufügen|erstellen)$/i,
      })
      .first();
    await expect(
      submit,
