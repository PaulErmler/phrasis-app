...
test.describe('chat (live)', { tag: '@live' }, () => {
  ...
  test('full chat-to-card flow: send → response → approve → quota decrements → card in library', async ({
    page,
  }) => {
    ...
    const emptyState = page.getByText(/no results|no sentences match/i);
    const visibleResult = page.getByText(new RegExp(searchSeed, 'i')).first();
  ...
  test('getThread + listThreads: thread persists and is listed (no extra messages)', async ({
    ...
    await expect(
      page.getByTestId('chat-assistant-message').first(),
    ).toBeVisible({
      timeout: 20_000,
    });
  ...
  test("word-explain flow: click a word on a learning card → Explain auto-submits 'Explain: <word>'", async ({
    page,
  }) => {
