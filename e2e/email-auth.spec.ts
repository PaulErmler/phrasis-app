...
test.describe('email/password auth journey', () => {
  test('sign-up routes to code entry and creates no session', async ({
    page,
  }) => {
  ...
  test('unverified sign-in re-sends the code and routes to code entry', async ({
    page,
  }) => {
  ...
  test('wrong code is rejected, correct code signs the user in', async ({
    page,
  }) => {
  ...
  test('6th auth email within the hour is rate-limited but UI stays generic', async ({
    page,
  }) => {
  ...
  test('forgot password for an unknown address leaks nothing', async ({
    page,
  }) => {
    ...
    await expect(page.getByText(/not found|no account|unknown/i)).toHaveCount(
      0,
    );
    await expect(
