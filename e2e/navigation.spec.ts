...
test.describe('authed navigation shell', () => {
  test('can move between home, library, stats, settings', async ({ page }) => {
    ...
    const targets: Array<{
      view: 'library' | 'stats' | 'settings' | 'home';
      urlFragment: RegExp;
    }> = [
  ...
  test('browser back/forward preserves view after BottomNav navigation', async ({
    page,
  }) => {
    await page.goto('/app');
    await page.waitForLoadState('domcontentloaded');
    ...
    await expect(page.getByTestId('course-menu-trigger').first()).toBeVisible({
      timeout: 10_000,
    });
